...
class EntityLinkingDataset(object):
    def __init__(self, dataset_dir, mention_db_path, wikipedia_titles_file=None, wikipedia_redirects_file=None):
        ...
        self.train = load_documents(os.path.join(dataset_dir, 'aida_train.csv'),
                                    os.path.join(dataset_dir, 'aida_train.txt'), person_names, self.mention_DB)
        self.test_a = load_documents(os.path.join(dataset_dir, 'aida_testA.csv'),
                                     os.path.join(dataset_dir, 'testa_testb_aggregate_original'), person_names, self.mention_DB)
        ...
        self.ace2004 = load_documents(os.path.join(dataset_dir, 'wned-ace2004.csv'),
                                      os.path.join(dataset_dir, 'ace2004.conll'), person_names, self.mention_DB)
        self.aquaint = load_documents(os.path.join(dataset_dir, 'wned-aquaint.csv'),
                                      os.path.join(dataset_dir, 'aquaint.conll'), person_names, self.mention_DB)
        self.clueweb = load_documents(os.path.join(dataset_dir, 'wned-clueweb.csv'),
                                      os.path.join(dataset_dir, 'clueweb.conll'), person_names, self.mention_DB)
        self.msnbc = load_documents(os.path.join(dataset_dir, 'wned-msnbc.csv'),
                                    os.path.join(dataset_dir, 'msnbc.conll'), person_names, self.mention_DB)
        self.wikipedia = load_documents(os.path.join(dataset_dir, 'wned-wikipedia.csv'),
                                        os.path.join(dataset_dir, 'wikipedia.conll'), person_names, self.mention_DB)
...
def load_documents(csv_path, conll_path, person_names, mention_DB):
    ...
    for (doc_name, mentions) in mention_data.items():
        ...
        for mention in mentions:
            ...
            while True:
                ...
                if doc_mention_text == mention_text:
                    ...
                    document['mentions'].append({'text':mention['text'], 'candidates':doc_mention_cand, 'title':mention['title'], 'start':doc_mention_span['start'], 'end':doc_mention_span['end']})
        ...
        while mention_span_index < len(document['mention_spans']):
            ...
            doc_mention_text = ' '.join(document['words'][doc_mention_span['start']:doc_mention_span['end']])
            doc_mention_cand = sorted([Candidate(m.title, m.prior_prob) for m in mention_DB.query(doc_mention_text)], key= lambda c: c.prior_prob, reverse=True)
            document['mentions'].append({'text':doc_mention_text, 'candidates':doc_mention_cand, 'title':'[NO_E]', 'start':doc_mention_span['start'], 'end':doc_mention_span['end']})
            mention_span_index += 1
...
def convert_documents_to_features(documents, tokenizer, entity_vocab, mode, document_split_mode, max_seq_length,
    ...
    for document in documents:
        ...
        for doc_start, doc_end in seq_record:
            ...
            output_mentions, feature_dict = generate_feature_dict(tokens, mention_data, doc_start - left_ctx_len, doc_end + right_ctx_len, doc_start, doc_end)
            if output_mentions:
