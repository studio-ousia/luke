...
@click.group(name="end2end-entity-linking")
def cli():
    pass
...
@click.option("--mentiondb-file", type=click.Path(exists=True), default="mention_db_from_p_e_m")
...
@click.option(
    "-t", "--test-set", default=["test_b", "ace2004", "aquaint", "msnbc", "wikipedia", "clueweb"], multiple=True
)
...
def run(common_args, **task_args):
    ...
    dataset = EntityLinkingDataset(args.data_dir, args.mentiondb_file, args.titles_file, args.redirects_file)
...
def create_candidate_list(dump_db_file, mention_db_file, out_file, data_dir):
    ...
    for documents in tqdm(reader.get_all_datasets()):
        ...
        # break
    # logger.info('***** Frozing *****')
    # entity_titles = frozenset(titles)
    # debug and TODO #
    # entity_vocab = {PAD_TOKEN: 0, MASK_TOKEN: 1, '[NO_E]': 2}
    # for n, title in enumerate(sorted(entity_titles), 3):
    #    entity_vocab[title] = n
    ##################
    ...
    # tk = BasicTokenizer()
    # train_data = convert_documents_to_features(
    #        reader.aquaint, tk, entity_vocab, 'train', 'simple', 500,
    #        30, 3)
