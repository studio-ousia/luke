...
@click.option(
    "--lr-schedule",
    type=click.Choice(["warmup_constant", "warmup_linear"]),
...
def run_pretraining(args):
    ...
    train_batch_size = int(
        args.batch_size / args.gradient_accumulation_steps / num_workers
    )
    ...
    if args.model_file is None:
        ...
        if args.wikipedia2vec_file:
            ...
            oov_titles = []
            for title, index in tqdm(
                dataset.entity_vocab.vocab.items(),
                disable=args.local_rank not in [0, -1],
            ):
                try:
                    entity_vector = torch.from_numpy(wiki2vec.get_entity_vector(title))
                    model.entity_embeddings.entity_embeddings.weight.data[index].copy_(
                        entity_vector
                    )
                except KeyError:
                    if not (
            ...
            for title in oov_titles:
                logging.info(
                    "Cannot load pretrained vector from Wikipedia2Vec: %s", title
                )
    ...
    def save_model(model, suffix):
        ...
        torch.save(
            optimizer.state_dict(), os.path.join(args.output_dir, optimizer_file)
        )
        scheduler_file = f"scheduler_{suffix}.bin"
        torch.save(
            scheduler.state_dict(), os.path.join(args.output_dir, scheduler_file)
        )
        metadata = dict(
    ...
    for batch in batch_generator.generate_batches():
        ...
        if accumulation_count == args.gradient_accumulation_steps:
            ...
            for name in ("masked_lm", "masked_entity"):
                try:
                    summary[name + "_loss"] = np.concatenate(
                        [r[name + "_loss"].flatten() for r in results]
                    ).mean()
                    correct = np.concatenate(
                        [r[name + "_correct"].flatten() for r in results]
                    ).sum()
                    total = np.concatenate(
                        [r[name + "_total"].flatten() for r in results]
                    ).sum()
                    if total > 0:
            ...
            if args.local_rank == -1 or worker_index == 0:
                ...
                if (
                    args.save_interval_sec
                    and time.time() - prev_save_time > args.save_interval_sec
                ):
